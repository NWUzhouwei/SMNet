...
class MLGA(nn.Module):
    ...
    def forward(self, lc_xyz, lc_x, knn_xyz, knn_x):
        ...
        if self.surface_points is not None:
            ...
            est_normal = self.norm_embedding(est_normal.permute(0, 2, 1),True)
            est_curvature = self.curv_embedding(est_curvature.permute(0, 2, 1),True)
...
class SA_Layer(nn.Module):
    def __init__(self, channels):
        ...
        # if channels>=4:
        #     self.q_conv = nn.Conv1d(self.channel,self.channel//4, 1, bias=False)
        #     self.k_conv = nn.Conv1d(self.channel, self.channel//4, 1, bias=False)
        # else:
        self.q_conv = nn.Conv1d(self.channel, self.channel, 1, bias=False)
        self.k_conv = nn.Conv1d(self.channel, self.channel, 1, bias=False)
        self.q_conv.weight = self.k_conv.weight
        self.q_conv.bias = self.k_conv.bias

        self.v_conv = nn.Conv1d(self.channel, self.channel, 1)
        self.trans_conv = nn.Conv1d(self.channel, self.channel, 1)
        self.after_norm = nn.BatchNorm1d(self.channel)
        self.act = nn.ReLU()
